...
function App() {
  ...
  const translations: Translations = {
    ...
    vi: {
      ...
      fieldsError: 'Vui lòng điền đầy đủ các trường.',
  ...
  const handleLogin = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setTimeout(() => setLoginMessage(''), 2000); // Instant disappear after 2 seconds
      }
    } catch (error) {
  ...
  // Handle password change submission (updated to remove oldPassword)
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (username && password && confirmPassword) {
      if (!passwordRegex.test(password)) {
        ...
        }, 10000); // Instant disappear after 10 seconds
        return;
      }
      if (password !== confirmPassword) {
        ...
        setTimeout(() => {
          ...
          setPassword(''); // Clear password input
          setConfirmPassword(''); // Clear confirm password input
        }, 3000); // Instant disappear after 3 seconds
        return;
      }
      try {
